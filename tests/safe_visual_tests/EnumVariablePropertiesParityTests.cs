...
public class EnumVariablePropertiesParityTests
{
    ...
    [Test]
    public async Task AnEnumVariable_OpensTheVariableDialog_NotTheTypeEditor()
    {
    ...
    /// <summary>Committing a state stores the chosen value's ID — not its name. This is the assertion that
    /// matters: a name here would be a broken IDREF that still looked right in the dialog.</summary>
    [Test]
    public async Task CommittingAState_StoresTheValuesIdREF()
    {
    ...
    /// <summary>And the variable's own NAME commits, which the type editor could never reach.</summary>
    [Test]
    public async Task CommittingANewName_RenamesTheVariable_NotTheType()
    {
        var (harness, vm, variable) = await WithEnumAsync();
        using var _1 = harness;
        harness.Dialogs.VariablePropertiesResult = new VariablePropertiesResult(
            "Persienne stue", string.Empty, ResourceInitialValue.OfChoice("Ukendt"), string.Empty);

        await vm.PropertiesCommand.ExecuteAsync(TreeNodes.FindPin(vm.InstallationNodes, "Persienne tilstand")!);

        Assert.That(harness.Session.Current!.FindById(variable)!.GetAttribute("name"), Is.EqualTo("Persienne stue"));
    }
