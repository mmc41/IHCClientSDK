...
public class TitleDirtyMarkerTests : AvaloniaTestBase
{
    ...
    public async Task Title_TracksDirtyBullet_AcrossEditUndoSaveTransitions()
    {
        ...
        Assert.That(vm.Title, Is.EqualTo("Untitled - IHC OpenVisual"), "a clean open shows no bullet");

        await harness.Session.AddLocalityAsync();
        Assert.That(vm.Title, Is.EqualTo("Untitled• - IHC OpenVisual"), "an edit sets the bullet (D07)");
