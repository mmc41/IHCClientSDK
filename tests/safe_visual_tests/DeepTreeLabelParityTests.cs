...
{
    private static async Task<TreeNodeViewModel> FunctionsRootAsync(ShellHarness harness)
    {
        string path = Path.Combine(TestContext.CurrentContext.TestDirectory, "testdata", "projects", "Project1-SimpelWired.vis");
        Project project = await harness.ProjectService.Load(path);
    ...
    public async Task ScenePin_ShowsItsNote_TruncatedTo15Characters()
    {
        ...
        TreeNodeViewModel root = await FunctionsRootAsync(harness);
