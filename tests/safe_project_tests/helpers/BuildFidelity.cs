...
namespace Ihc.Vis.Tests
{
    ...
    /// reported byte offset + line/col, fix the one divergence, repeat) and centralizes the SDK-embedded catalog the
    /// content builds author against. Complements <see cref="ProjectByteFidelityTests"/> (which proves the
    /// <em>round-trip</em> path) by proving the <em>builder</em> emits vendor-faithful output, not merely
    /// structurally-equal output.
    /// </summary>
    internal static class BuildFidelity
    {
        /// <summary>The SDK-embedded catalog the content builds author against — no IHC Visual install needed.</summary>
        public static ICatalog RequireCatalog() => new BuiltInCatalog();
