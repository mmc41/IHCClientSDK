...
internal static class RebuildEquivalenceOracle
{
    ...
    // This law's own operation set: it is about the change classes a reconciler must handle, so it has no
    // history operations - undo is not a tree change the projector sees differently from any other edit.
    private static readonly Gen<Op> AnyOp = Gen.OneOf(
        CsCheckValues.Name.Select(n => (Op)new AddLocalityOp(n)),
        Gen.Select(CsCheckValues.Pick, CsCheckValues.Name, (pick, name) => (Op)new RenameOp(pick, name)),
        Gen.Select(CsCheckValues.Pick, CsCheckValues.Pick, (pick, pos) => (Op)new ReorderOp(pick, pos)),
        CsCheckValues.Pick.Select(pick => (Op)new DeleteOp(pick)));
    ...
    internal static readonly Gen<Op[]> CommandSequence = Gen.Select(
        CsCheckValues.Name, AnyOp.Array[0, 7], (first, rest) => (Op[])[new AddLocalityOp(first), .. rest]);
