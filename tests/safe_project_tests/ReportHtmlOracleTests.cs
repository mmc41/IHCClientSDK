...
/// <c>&lt;symbol&gt;</c> sprite and <c>#icon-logo</c> banner, so the HTML byte contract belongs to a suite that
/// references the app assembly, beside the reports it is about rather than in an SDK-only one. The provider
...
/// matrix, oracle naming, pinned clock and byte assert are the shared <see cref="ReportOracleHarness"/> harness.
/// </summary>
