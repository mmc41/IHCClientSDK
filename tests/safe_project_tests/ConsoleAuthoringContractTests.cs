...
namespace Ihc.Vis.Tests
{
    ...
    /// <see cref="ProjectCommands"/> gateway and a <see cref="ProjectDocumentSession"/> — with NO
    ...
    public class ConsoleAuthoringContractTests
    {
        ...
        public async Task AuthoringDoor_Load_Command_Apply_Validate_Save_WithoutAnyGui()
        {
            ...
            // The console runner: a stateless document session over the loaded project (the SDK apply path).
            var session = new ProjectDocumentSession();
            session.Open(project, startClean: true);

            // 1. A context-free command obtained through the one door, then applied.
            EditOutcome<ElementId> localityOutcome = session.Apply(app.Commands.AddLocality(session.Current!, "Console Room"));
            ...
            AddProduct? productCommand = app.Commands.AddProduct(session.Current!, localityId, productDef.ProductIdentifier);
            Assert.That(productCommand, Is.Not.Null, "the door resolved the catalog product");
            EditOutcome<ElementId> productOutcome = session.Apply(productCommand!);
            Assert.That(productOutcome.Status, Is.EqualTo(EditStatus.Committed), "the gateway product command commits");

            Project edited = session.Current!;
