...
public class ProblemsPanelViewModelTests
{
    ...
    /// <summary>
    /// The list is DERIVED from the document rather than cached beside it: a row never outlives the element it
    /// names, and undoing the deletion brings the finding back.
    /// </summary>
    /// <remarks>
    /// The validation body here is a FUNCTION of the project rather than a fixed list, which is what makes the
    /// round trip mean anything: nothing tells the panel to drop a row, so a panel holding its own copy would
    /// still be showing one for an element that is gone -- a journey to nowhere offered to the installer.
    /// </remarks>
    [Test]
    public async Task ARowDropsWithItsElementAndComesBackWhenTheDeleteIsUndone()
    {
        using Rig rig = new();
        rig.Body = project => System.Collections.Immutable.ImmutableArray.CreateRange(
            project.Groups.Select(_ => Finding(
                "doc-name-empty", ValidationSeverity.Warning, "Lokaliteten mangler navn.")));
        await rig.OpenAsync();
        await rig.Harness.Session.ApplyAsync(new AddLocality("Slettes igen"));
        await rig.SettleAsync();

        int withIt = rig.Panel.Rows.Count;
        ElementId doomed = rig.Harness.Session.Current!.Groups[^1].Id!.Value;
        Assert.That(withIt, Is.GreaterThan(0), "precondition: the locality produced a row to lose");

        await rig.Harness.Session.ApplyAsync(new DeleteLocality(doomed));
        await rig.SettleAsync();
        Assert.That(rig.Panel.Rows, Has.Count.EqualTo(withIt - 1),
            "the row named an element that is gone, so the re-validated list no longer carries it");

        await rig.Harness.Session.UndoAsync();
        await rig.SettleAsync();
        Assert.That(rig.Panel.Rows, Has.Count.EqualTo(withIt),
            "and undoing the delete brings the finding back");
    }
    [Test]
