...
| `validation/` | the validation-engine oracles (`.xml`) — the per-case finding recording under `validation/findings/` (one file per corpus case) and the severity×operation matrix beside it; also *derived*, the recording by an `[Explicit]` regeneration test and the matrix from the catalogue and a source scan |
...
Two kinds of artifact pinning what the validation engine **says** and how that relates to what the code
**does**: the eighteen XML files under `findings/`, one per corpus case, and one compact XML disagreement
set. Both are *derived, not captured*, but they are adopted differently: the findings oracles are rewritten
wholesale by an `[Explicit]` regeneration test and diffed, while the disagreement set is small enough that
its gate prints the expected rows on failure and adopting a REVIEWED change is pasting them.

The two differ in what "correct" means, and the difference matters when one of them fails. The findings
oracles ARE byte-fidelity oracles, exactly like the `.vis` and report oracles: the gate is byte equality per
case. The disagreement set is a *fitness gate* compared structurally — its rows are the content, its bytes
are not.
...
| `severity-operation-matrix.xml` | 7 | one `<divergence code="…" kind="…"/>` per row | `SeverityOperationMatrixTests.TheDerivedDisagreementSetMatchesTheRecordedFile` | pasting the rows the failing gate prints |
...
### severity-operation-matrix.xml — the disagreement set

The catalogue rows whose **disposition** and **operation** disagree. The disposition is derived from two
places at once — *published* (what `ihcclient/docs/problem-catalogue.md` says, generated regions excluded)
and *declared* (what the entry in code says) — and the operation from what the SDK can actually raise. Seven
rows today, in three kinds: four `refuses-but-not-fatal` rows that are **two faces, not a contradiction**
(they report at validate and refuse at the operation, both intended), two `fatal-but-proceeds` rows where the
catalogue promises a refusal the code does not make — left that way by product decision D13, because
introducing a refusal is a ruling rather than a task — and one `ruled-out` row.

Rows that **agree are not listed**, so an empty file would mean the catalogue and the code say the same thing
everywhere. A row records only what identifies a disagreement — its `code` and its `kind` — deliberately: no
source paths, line numbers or derived prose, so an edit that moves lines or rewords a decision cannot stale
the file. When the derived set really changes, the gate fails and **prints the expected rows**; adopting a
reviewed change is pasting them into the file in the same commit as the change that moved them.

### Encoding and layout

`findings/*.xml` is ISO-8859-1, CRLF and BOM-less, pinned by `.gitattributes` (see above).
`severity-operation-matrix.xml` is **UTF-8 without BOM** and deliberately **unpinned** — its gate compares
rows through an XML parse, not bytes, so line endings cannot matter to it. `tests/TestData.props` copies both
into the consuming suite's output via its `**\*.xml` glob; the matrix is read through
`TestData.PathOf("validation/severity-operation-matrix.xml")` and the findings through the shared
`FindingOracleHarness`. Three places need the *source* checkout instead and go through
`TestRepository.RequireRoot()`: the matrix derivation (it reads the problem catalogue's markdown and scans
`ihcclient/src` for raised refusal identities), `SessionRefusalPolicyTests` (it scans the session layer's
sources), and `CatalogCompletenessTests`. The findings regenerator is `[Explicit]` and writes into
`findings.generated/` beside the test binary, so the copy across is manual and deliberate.
...
- Files are copied to each consuming suite's output as `testdata\...` by `tests/TestData.props`, which
  ...
  report oracles, and `*.xml` for the findings oracles and the disagreement set). Access them through the
  `TestData` helper for a path (`projects/LiveAuthored/step02-pir2.vis`,
  `validation/severity-operation-matrix.xml`) or through `FindingOracleHarness` for a findings oracle —
...
- Line endings are pinned in `.gitattributes`: **CRLF** for `*.vis`/`*.def`/`*.ifb` and
  `validation/findings/*.xml`, **LF** for `reports/*`. `validation/severity-operation-matrix.xml` is
  deliberately **unpinned** — nothing compares it byte-for-byte. Never renormalize or rewrite the committed
  blobs.
