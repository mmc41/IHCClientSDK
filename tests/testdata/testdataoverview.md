...
| `validation/` | the four validation-engine oracles (`.txt`) — the finding recording, its rule-id map, the error-origin inventory and the severity×operation matrix; also *derived*, three of them by an `[Explicit]` regeneration test and the inventory by a source scan |
...
150 of the 618 findings recorded in `validation/rule-characterization.txt`, under the case name
`fixture/Project6-Errors` rather than `authentic/…` precisely because its defects are intended.
...
- **DuplicatedAdressErrors.vis** — the `dataline-address-duplicate` witness: two inputs on data-line
  ...
  from this file, so the duplicate-address rows in `validation/rule-characterization.txt` come from the
  in-code fixture, not from here. The file is kept as the catalogue's cited oracle; wiring it into the
  corpus would move that recording and needs the regenerate-and-diff procedure below.
...
(exactly its 12 `Documentation` findings in `validation/rule-characterization.txt`: the punch-list that
...
both `validation/rule-characterization.txt` **and** all twelve `full-*` files here — the two-oracle rule in
...
Four tab-separated `.txt` artifacts pinning what the validation engine **says** and where the SDK's `.vis`
surface **says it**. Like `reports/` they are *derived, not captured*, and none may be hand-edited: three are
rewritten wholesale by an `[Explicit]` regeneration test, and the fourth — `error-origins.txt`, whose
classification column is a judgement no generator can make — is kept honest by a source scan instead. Unlike
`reports/` they are not byte-fidelity oracles: they are *fitness gates*, and each one's real content is the
classification in its columns, not its size. Every file opens with a comment block stating its own scope and
regeneration rule; read that header before touching it.

| File | Rows | Shape | Gate | Regenerated by |
|------|------|-------|------|----------------|
| `rule-characterization.txt` | 618 findings over 18 corpus cases | `case ⇥ severity ⇥ rule-id ⇥ category ⇥ locator ⇥ message` | `ValidationCharacterizationTests.Corpus_ReproducesTheRecordedFindings` | `Regenerate_TheRecording` |
| `rule-id-map.txt` | 93 (one per distinct rule id) | `recorded-id ⇥ emitted-id(s) ⇥ emitted-message ⇥ emitted-category` | `ValidationCharacterizationTests.Map_GovernsExactlyTheRecordedRuleIds` | `Regenerate_TheRecording` (same run) |
| `error-origins.txt` | 46 | `file ⇥ shape ⇥ count ⇥ family ⇥ classification` | `ErrorOriginInventoryTests` (4 tests) | hand-classified; the scan is the gate |
| `severity-operation-matrix.txt` | 7 | `code ⇥ published ⇥ declared ⇥ operation ⇥ site ⇥ face ⇥ divergence ⇥ decision` | `SeverityOperationMatrixTests.TheDerivedMatrixMatchesTheCheckedInCopy` | `Regenerate_TheMatrix` |

### rule-characterization.txt — the finding recording

Every finding the SDK produces over the whole validation corpus, **in production order**, one per line. 618
findings: 28 `Error` and 590 `Warning`, spread over eight categories (`Documentation` 214, `Logic` 145,
`ProjectStructure` 76, `Wiring` 64, `FileIntegrity` 34, `Scenes` 33, `Addressing` 26, `DeviceSettings` 26) and
93 distinct rule ids. The locator is `-` when a finding names no element.
...
⚠ **Never hand-edit it to make a migration pass.** An intended rule-id, message or category change is
*declared* in `rule-id-map.txt` first; then the recording is regenerated and the diff reviewed. `CLAUDE.md`
states the adoption rule: every changed line must be explained by a rule that changed in the same edit.
`Regenerate_TheRecording` writes `rule-characterization.generated.txt` and `rule-id-map.generated.txt` into the
test binary directory rather than over the committed copies — copying them across is the deliberate act.

Four other fixtures read this file rather than re-deriving what it holds, so it is load-bearing beyond its own
gate: `MigrationParity` (per-rule parity for the Phase-5 migrations), `SchemaRuleParityTests`,
`CatalogCompletenessTests` (checks each catalogue entry's disposition against the severity the recording shows)
and `DatalineAddressRemapTests` (asserts the three `dataline-address-*` successors appear and the retired
`dataline-address` id does not).

### rule-id-map.txt — the declared-change map

One row per distinct recorded rule id, and today **every row reads `=` in all three cells**. That is the
post-migration state, not a disabled gate: the recording was re-made once when the pipeline switched to the
engine, so recorded and produced agree again. The columns stay because the *next* change needs them — and
because a change made without declaring it here still fails `Corpus_ReproducesTheRecordedFindings`. The row
count is pinned a second way, in code: `ValidationCharacterizationTests.BaselineRuleIdCount` is 93, and its
doc-comment carries the running commentary on which rule took the count to each value.

### error-origins.txt — the error-origin inventory

Every place the SDK's `.vis` surface tells someone something went wrong, classified as one of exactly three
things: a **Refusal** (a Danish sentence saying an operation will not be carried out — needs a code), a
**Failure** (something broke; English to the log, fixed Danish label to the user — no code of its own) or a
**Finding** (a report about a file's content — coded already). Scope is `ihcclient/src/vis/` plus
`ProjectAppService.cs`; the controller API tier is deliberately outside it, and ~100 argument guards are
excluded by design because an out-of-contract argument is a bug in the caller, not an outcome a user can act
on.

This is the one file here that is **not machine-regenerated** — the classification is a judgement. Instead
`ErrorOriginInventoryTests.EveryOriginInTheSourceIsInTheInventory` re-scans the source tree for the six
origin shapes (`edit-verdict-refusal` = `EditVerdict.Refuse(`, `deep-guard-refusal` = `EditRefusedException`,
`preview-refusal` = `PreviewOutcome.Refused(`, `operation-throw` = the four plain exception types,
`coded-refusal` = the five coded exception types, `validation-finding` = `new ProjectValidationFinding(`) and
fails on any row whose file/shape/**count** disagrees. The counts are what make it a gate rather than a list:
an uncoded
refusal cannot be added quietly. A companion test pins the refusal population at **34 edit-family refusals**
(25 verdict sites, 6 deep guards, 3 preview refusals).

Because it scans the checkout rather than the build output, this gate moves whenever refusal or throw sites
move — including when a file is renamed or deleted. Reclassify and update the rows in the same edit that
moves the code.

### severity-operation-matrix.txt — the disagreement set

The catalogue rows whose **disposition** and **operation** disagree, across three independent facts:
*published* (what `ihcclient/docs/problem-catalogue.md` says), *declared* (what the entry in code says) and
*operation* (what the code actually does on meeting the condition). Seven rows today, in two divergence
classes: four `refuses-but-not-fatal` rows in `ProjectSerializer.cs` that are **two faces, not a
contradiction** (they report at validate and refuse at the operation, both intended), and three
`fatal-but-proceeds`/`ruled-out` rows where the catalogue promises a refusal the code does not make — left
that way by product decision D13, because introducing a refusal is a ruling rather than a task.

Rows that **agree are not listed**, so an empty file would mean the catalogue and the code say the same thing
everywhere. Two rows carry `ProjectSerializer.cs:NN` line numbers, so an edit that shifts lines in that file
moves this oracle even when nothing about the behaviour changed — regenerate with `Regenerate_TheMatrix` and
check the diff is line numbers only.

### Encoding and layout

All four are **UTF-8 without BOM and LF**, and they are the one group under `tests/testdata/` with **no
`.gitattributes` pin** — deliberately, because nothing compares them byte-for-byte. The readers are
CR-tolerant (`File.ReadAllLines`, or an explicit `\r\n` → `\n` fold on the oracle side), so a CRLF checkout
still passes. `tests/TestData.props` copies them into the consuming suite's output via its `**\*.txt` glob —
the same glob that carries the report oracles — and they are read through `TestData.PathOf("validation", …)`.
Three places need the *source* checkout instead and go through `TestRepository.RequireRoot()`: the origin
scan, `Regenerate_TheMatrix` (which writes over the committed file directly), and `CatalogCompletenessTests`.
`Regenerate_TheRecording` is the exception — it writes its two `*.generated.txt` files into
`TestContext.CurrentContext.TestDirectory`, so the copy across is manual and deliberate (it also carries
`[Category("OracleRegeneration")]`). Both regeneration tests are `[Explicit]`, so a normal run never fires
them.
...
- Files are copied to each consuming suite's output as `testdata\...` by `tests/TestData.props`, which
  ...
  report **and** validation oracles). Access them through the `TestData` helper (relative paths like
  `projects/LiveAuthored/step02-pir2.vis` or `validation/rule-characterization.txt`) — never by
  walking up to the source checkout. A new suite needing oracles adds the one `<Import>` line. The `.md`
...
- Line endings are pinned in `.gitattributes`: **CRLF** for `*.vis`/`*.def`/`*.ifb`, **LF** for
  `reports/*`. `validation/*.txt` is deliberately **unpinned** — nothing compares it byte-for-byte and its
  readers tolerate CR. Never renormalize or rewrite the committed blobs.
