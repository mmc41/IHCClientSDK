...
`safe_visual_tests`. The `.vis` files under `projects/` are the byte-oracles for `safe_project_tests`. Every file directly in
...
- Files are copied to each consuming suite's output as `testdata\...` by `tests/TestData.props`, which
  those suites `<Import>` (`*.vis` plus `*.def`/`*.ifb` for the catalog oracles and `*.html` for the report
  oracles). Access them through the `TestData` helper (relative paths like
