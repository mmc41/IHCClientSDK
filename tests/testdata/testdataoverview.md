...
| `validation/` | the validation-engine oracles (`.xml`) — the per-case finding recording, one file per corpus case directly in `validation/`; *derived*, by an `[Explicit]` regeneration test |
...
editing actions in one session (single save); the actions are fully specified below (capture session
`mutoracle`, 2026-07-04). Its purpose is the **authoring-pipeline byte-fidelity**
...
copy→paste actions in one session (single save); the actions are fully specified below (capture session
`cporacle`, 2026-07-04). Its purpose is the **copy/paste byte-fidelity** gate for
...
authoring action in a single session (single save; capture session `cporacle3`, 2026-07-04). Because the
...
action in a single session (single save; capture session `ENG-A1`, 2026-07-11). Its
...
drags in a single session (single save; capture session `ENG-A2`, 2026-07-11). Its
...
action in a single session (single save; capture session `ENG-A3`, 2026-07-11). Its
...
edits) in one session (capture session `ENG2-A5`, 2026-07-12). It pins **project2's
...
session (single save; capture session `ENG2-A5`, 2026-07-12). Its purpose is the
...
sequence in a single session (single save; capture session `ENG2-B1`, 2026-07-12). First oracle in the
corpus with a **NOT condition (`method="_0x28"`)** and a
...
Authentic IHC Visual output **derived from `project2-CustomBlock.vis`** by one recorded program-authoring
sequence in a single session (single save; capture session `ENG2-B2`, 2026-07-12). First oracle in the
corpus with a **`program_case`/`case_action`** structure. Its purpose is
...
in a single session (single save; capture session `ENG2-A4B`, 2026-07-12). Its purpose
...
one recorded session (capture session `e2`, 2026-07-21; the tokens it attests are listed below and
carried in `ihcclient/src/vis/programs/ProgramMethodCatalog.cs`). One unlocked FB **"Tom blok"** in Stue with
...
session (capture session `progmode3`, 2026-07-21). The base FB "Tom blok" (Stue) gained 10 rows — one
control action, two events
...
project3's Stue), with dialog Navn=`GemOracle` and Note=`Oracle tooltip` (capture session `ENG-A4`,
2026-07-11). Its purpose is the **export-placed-block byte gate** (backlog G5,
...
through the shared `FindingOracleHarness`. Some suites need the *source* checkout instead and go through
