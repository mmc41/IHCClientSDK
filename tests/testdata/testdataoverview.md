...
150 of the 618 findings recorded under `validation/findings/`, in its own file
...
- **DuplicatedAdressErrors.vis** — the `dataline-address-duplicate` witness: two inputs on data-line
  ...
  from this file, so the duplicate-address rows in `validation/findings/synthetic-dataline-address.xml` come
...
(exactly its 12 `Documentation` findings in `validation/findings/`: the punch-list that
...
both that case's file under `validation/findings/` **and** all twelve `full-*` files here — the two-oracle rule in
...
Two kinds of artifact pinning what the validation engine **says** and where the SDK's `.vis` surface **says
it**: the eighteen XML files under `findings/`, one per corpus case, and two tab-separated `.txt` inventories.
All of them are *derived, not captured*, and none may be hand-edited — the findings oracles and
`severity-operation-matrix.txt` are rewritten wholesale by an `[Explicit]` regeneration test, while
`error-origins.txt`, whose classification column is a judgement no generator can make, is kept honest by a
source scan instead.

The two families differ in what "correct" means, and the difference matters when one of them fails. The
findings oracles ARE byte-fidelity oracles, exactly like the `.vis` and report oracles: the gate is byte
equality per case. The two `.txt` inventories are *fitness gates*, whose real content is the classification in
their columns rather than their bytes. Every `.txt` opens with a comment block stating its own scope and
regeneration rule; read that header before touching it.

| File | Rows | Shape | Gate | Regenerated by |
|------|------|-------|------|----------------|
| `findings/*.xml` | 618 findings over 18 corpus cases, one file each | flat, attribute-only XML: a root carrying the run's own caveats, then one `<finding …/>` per line | `ValidationCharacterizationTests.Corpus_ReproducesItsOracleByteForByte` (per case), plus `FindingOracleConformanceTests` and `FindingOracleCoverageTests` | `Regenerate_TheFindingsOracles` |
...
### findings/ — the finding recording
...
(`tests/shared/FindingOracles.cs`, linked into both consuming suites) so a change to the format is a compile
...
- Files are copied to each consuming suite's output as `testdata\...` by `tests/TestData.props`, which
  ...
  report and inventory oracles, and `*.xml` for the findings oracles). Access them through the `TestData`
  helper for a path (`projects/LiveAuthored/step02-pir2.vis`, `validation/error-origins.txt`) or through
  `FindingOracles` for a findings oracle — never by walking up to the source checkout. A new suite needing oracles adds the one `<Import>` line. The `.md`
