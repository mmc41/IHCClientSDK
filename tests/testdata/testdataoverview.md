...
`safe_visual_tests`. It has five subfolders:

| Folder | Contents |
|--------|----------|
| `projects/` | `.vis` project oracles — authentic vendor output directly in `projects/` and in `projects/LiveAuthored/`, hand-crafted files in `projects/Synthetic/`; plus the two authoring records `project5-Dokumentation.md` and `Project6-Errors.md`, and `Project6-Error.report.pdf` |
...
| `reports/` | the 24 generated documentation-report oracles (`.html` + `.txt`) — *derived* fixtures, regenerated by the SDK rather than captured from the vendor |
| `validation/` | the validation-engine oracles (`.xml`) — the per-case finding recording, one file per corpus case directly in `validation/`; *derived*, by an `[Explicit]` regeneration test. Schema-validated at build time against the SDK's `ihc_project_findings.xsd` — see [The schema](#the-schema-ihc_project_findingsxsd) |
...
editing actions in one session (single save); the actions are fully specified below. Its purpose is the
**authoring-pipeline byte-fidelity** gate: the SDK loads the *original*, replays these actions through the
public builder API (clock pinned to this file's stamp `id2="_0x40c1836"` / `modified 2026-07-04 12:24`),
and asserts byte-identity. `id1` is unchanged (`_0x1d0e2923`); `last_unique_id` rises `_0x56c → _0x59e`.
Note a load-time **Action 0**: a bare save of the original already re-hoists its two catalog
(`[read only]`, `typeid`) enums `Persienne tilstand`/`Logning` to the bottom of the enum block with
fresh ids `_0x56d..0x579` and rewrites their 4 `resource_enum` refs — a one-time normalization the
replay must reproduce (the SDK's passive load preserves the original's low enum ids). The three
mutations (in allocation order):
...
copy→paste actions in one session (single save); the actions are fully specified below. Its purpose is the
**copy/paste byte-fidelity** gate for `ProjectEditor.CopySubtree`: the SDK loads the *original*, replays
Action 0 + these copy actions through `CopySubtree`, saves with the clock pinned to this file's stamp
`id2="_0x40f391d"` / `modified 2026-07-04 15:57`, and asserts byte-identity. `id1` is unchanged (`_0x1d0e2923`);
...
driver (probed in the same session: `node_paste` of a function block adds no node and the following save
is control-identical), so that branch has no vendor oracle — the function-block `CopySubtree` path stays
SDK-defined rather than vendor-pinned.
...
authoring action in a single session (single save). Because the `ihcvisual` driver has
**no value-add verb** (only `enum.createType`/`listTypes`/`listValues`, all valueless), the enum was
authored directly in the live **"Enumerator typer og værdier"** dialog (command 24588) via Win32
...
action in a single session (single save). Its purpose is the
**Projektinfo/metadata authoring byte-fidelity** gate (backlog G4, US-039): the SDK loads the
...
drags in a single session (single save). Its purpose is the
**scene-link authoring byte-fidelity** gate (backlog G1, US-024): the SDK loads the
...
action in a single session (single save). Its purpose is the
**append-values-to-an-existing-enum byte-fidelity** gate (backlog M-A part 2, US-030
...
edits) in one session. It pins **project2's "Action 0"**: on load, IHC Visual re-hoists the two
`typeid`-bearing catalog enums to the enum-block bottom — Persienne `_0x4447→_0xf847`, Logning
`_0x4a47→_0xfe47` — and repoints every `resource_enum@typedef`;
...
session (single save). Its purpose is the **reference-cascade delete byte-fidelity** gate (backlog M-B,
US-009 program-reference half): the SDK loads the *original*, replays Action 0 + the delete below
through `DeleteById(…, CascadeReferences)`, saves with the clock pinned to `id2="_0xc0d0414"` /
`modified 2026-07-12 13:04:20`, and asserts byte-identity. `id1` unchanged; `last_unique_id` stays
**`_0x104`** (delete allocates 0 ids). Action 0 applies first. The single
action:
...
sequence in a single session (single save). First oracle in the corpus with a **NOT condition
(`method="_0x28"`)** and a **nested conditions group**. Its purpose is the **condition-logic-groups
byte-fidelity** gate (backlog G2, US-029): the SDK loads the *original*, replays Action 0 + the sequence
below through the `ConditionsGroupRef` surface, saves with the clock pinned to `id2="_0xc0e2338"` /
...
Authentic IHC Visual output **derived from `project2-CustomBlock.vis`** by one recorded program-authoring
sequence in a single session (single save). First oracle in the corpus with a
**`program_case`/`case_action`** structure. Its purpose is the **case-structure byte-fidelity** gate
(backlog G3, US-031): the SDK loads the *original*, replays Action 0 + the sequence below through the
`AddCase`/`CaseRef` surface, saves with the clock pinned to `id2="_0xc0e2c02"` /
`modified 2026-07-12 14:44:02`, and asserts byte-identity. `id1` unchanged;
...
in a single session (single save). Its purpose is pinning the
**in-document side effect of exporting a block to the library** (backlog G5 §18
...
one recorded session (the tokens it attests are listed below and carried in
`ihcclient/src/vis/programs/ProgramMethodCatalog.cs`). One unlocked FB **"Tom blok"** in Stue with
...
(enablement rule undetermined; its token remains unknown — `Tal = Tal /` does commit and binds a float
divisor, so the refusal is asymmetric and float-÷ must NOT be assumed equal to `_0x73`; the vendor's
refusal is settled by owner ruling under `-round2` below). Passive
...
session. The base FB "Tom blok" (Stue) gained 10 rows — one control action, two events and seven
conditions (in the pre-existing `program_sub`) — and nine single-purpose probe/pair-rig FBs
...
Beside it sits **`Project6-Error.report.pdf`** (104 KB, 4 pages) — the *vendor's own* documentation report for
this fixture, printed to PDF from IHC Visual during the same authoring session. It is a **reference capture,
not a test oracle**: nothing reads it, `tests/TestData.props` does not copy it (the glob takes only
`.vis`/`.def`/`.ifb`/`.html`/`.txt`), and no assertion compares against it. It is kept because it is the only
...
Hand-crafted files, mostly for `OpenWorldTests`; they exercise behavior no vendor file can, and are *not*
IHC Visual output.
...
- **DuplicatedAdressErrors.vis** — the `dataline-address-duplicate` witness: two inputs on data-line
  address 1.01, two more on 1.02 and two outputs on 1.01. Necessarily synthetic, and the problem
  catalogue says why — IHC Visual offers an already-claimed address as `N (i brug)` but disables OK, so
  no legal vendor edit can author this state (it is a ⊘ *reclassified* row in
  [`problem-catalogue.md`](../../ihcclient/docs/problem-catalogue.md) §5, which names this file as its
  oracle). ⚠ **Currently unreferenced by any test.** The validation corpus in
  `ValidationCharacterizationTests` builds its `synthetic/dataline-address` case *in code* rather than
  from this file, so the duplicate-address rows in `validation/synthetic-dataline-address.xml` come
  from the in-code fixture, not from here. The file is kept as the catalogue's cited oracle; wiring it into
  the corpus would move that recording and needs the regenerate-and-diff procedure below.

## Synthetic Product oracles (`products/synthetic/`) — not authentic

Fourteen hand-authored synthetic `.def` product files (not IHC Visual output) — the committed oracles for the
code-authoring builder surface (`ProductDefinitionBuilder`) **and** for the catalog file format itself. The
thirteen in the table below (`9f01`–`9f13`) are each consumed by committed tests three ways: the W1
reader→writer byte gate (`CatalogFileWriterTests`), the
...
The fourteenth is not a builder oracle at all and so has no table row — it is consumed by the dialog and
end-to-end fixtures instead:
...
- **`synthetic_9f14_unknownfamily.def`** — the OPEN-WORLD fixture. Its root tag `product_unknown_family`
  ...
  preset and the dialog composer's minimal fallback is what a caller gets (`ProductDialogComposerTests`
  imports it for exactly that). It **is** importable, insertable
...
project3's Stue), with dialog Navn=`GemOracle` and Note=`Oracle tooltip`. Its purpose is the
**export-placed-block byte gate** (backlog G5, US-021 "Gem…"):
`FunctionBlockRef.ExportDefinition` lifts the placed project block to a
...
`ReportOracleHarness.Cases` rather than retyped, and a missing combination surfaces as a missing file, not a missing
test row.
...
**`Fejl i dokumentation`** appendix carrying the Documentation-category findings — **12 rows for project5**
(exactly its 12 `Documentation` findings in `validation/`: the punch-list that
fixture exists to fire) and **77 rows for project3-enduserdoc**. The appendix is identical across all three
report kinds; only the body above it differs.

⚠ **This appendix is why a validation change moves report oracles.** The appendix *is* the rendering of the
Documentation category, so a rule that adds, removes, reclassifies or re-words a `Documentation` finding moves
both that case's file under `validation/` **and** all twelve `full-*` files here — the two-oracle rule in
`CLAUDE.md`. A `std-*` file never moves for a validation change; if one does, the change was not confined to
validation.
...
empty rather than leading a reader to a description of something that no longer exists). It recorded the
catalogue rows whose *disposition* and *operation behaviour* disagreed, derived from the entries, the
hand-written severity cells of `problem-catalogue.md` and a reflection scan of five refusal registries. It is
RETIRED, and deliberately so: its comment advertised that an empty file would mean "the catalogue and the
code say the same thing everywhere", while what it actually detected was far narrower — declared divergences
among a hard-coded five-type roster, plus one published severity cell. A gate that reads as guaranteeing more
than it checks invites over-trust. What replaced it, and what was consciously given up:

- `LoadRefusalTests.AProjectAboveVersionFourStillOpensToday` pins the one behavioural fact those rows rested
  on that nothing else pinned — a `version_major="5"` byte stream still OPENS. Closing that gap is
  product decision D13, and this test is the tripwire that forces the ruling to be made consciously.
- `RefusalLabelDriftTests.TheCausesRefusedUnderANonRefusalEntryAreExactlyTheAcknowledgedSet` keeps the
  acknowledgment semantic — a cause that refuses while its entry reports must be written down with its reason
  — and reflects over EVERY registry in the assembly rather than a fixed roster.
- The individual rows stay pinned by the suites that own them: `CatalogCompletenessTests.KnownUnimplemented`
  and `ImportBridgeRefusalTests` for `import-catalog-wrong-kind`, `LoadRefusalTests` and `ProblemCatalogTests`
  for `load-truncated`, and one execution test per cause in `SaveRefusalTests`, `EditOpenRefusalTests` and
  `AttributePolicyTests` for the two-faced schema rows.
- **Given up, knowingly:** nothing machine-reads §4's hand-written severity cells any more (the GENERATED
  index remains the gated severity publication); population changes no longer surface as one reviewed diff of
  one artifact; and a registry member that is declared but never referenced in `ihcclient/src` is no longer
  detected. Do not re-add a count gate, a throw inventory, or any source-path/line-coupled artifact to
  recover these.

A third artifact used to sit here: `error-origins.txt`, a hand-classified inventory of every throw site in
the `.vis` engine, kept honest by a per-file count scan. It is RETIRED: every user-facing refusal channel
(`EditVerdict.Refuse`, `EditRefusedException`, `PreviewOutcome.Refused`, the coded IO exceptions) demands a
coded problem by construction, so the count bookkeeping bought churn rather than detection. The one layer
fact worth keeping — the session layer refuses only through those typed channels, never a plain BCL throw —
moved into `SessionRefusalPolicyTests`, which needs no data file.
...
They are also a real EXPORT, not a test-only rendering: each is exactly what
`ProjectAppService.ExportFindings` writes for that case, with `SourceName` as the only non-default option.
That is what makes them worth reading — the format a user gets is the format pinned here.

#### The schema (`ihc_project_findings.xsd`)

The format has a published grammar: **`ihcclient/schemas/ihc_project_findings.xsd`**. It describes the SDK's
export format, so it ships with the SDK — packed into the NuGet package under `schemas/` — rather than with
the fixtures. There is exactly ONE copy in the repository. It covers the value vocabularies (the two enums,
the code grammar, locators, positional paths, the timestamp shape) and the complete attribute list, including
every `arg_*` slot `ProblemCatalog` declares.

⚠ **The oracle files carry NO schema reference of their own, deliberately.** An
`xsi:noNamespaceSchemaLocation` on the root would put two more attributes on the one line of these files that
is read by eye in every regeneration diff, and — because such a hint resolves against the document's own
...
⚠ **That build target is the ONLY gate on the grammar.** No test asserts it, so removing the import loses the
check silently. An editor can be pointed at the schema as well, through whatever per-file XML-schema
association it offers; that is a local convenience and no substitute for the build gate.

Because the schema is not referenced from the files, a consumer validating an **exported** findings file
supplies it themselves — take it from the NuGet package's `schemas/` folder or from `ihcclient/schemas/`.

Three parts of the contract are invisible to any validator, because they survive parsing, and are asserted on
the bytes instead: the ISO-8859-1 declaration with no BOM and CRLF ends (`FindingOracleConformanceTests`),
attribute ORDER (`FindingExportWriterTests`), and the rule that `@related_xpath` appears only beside
...
`projects/project4-PrgTokens.vis` is a committed `.vis` that is **not** a corpus case, and its absence is
...
A case is referenced in more places than its own byte test, so removing one is an edit in several places
rather than a file deletion:
...
by a rule that changed in the same edit — the adoption rule `CLAUDE.md` states.
`Regenerate_TheFindingsOracles` writes into `findings.generated/` beside the test binary rather than over the
committed copies; copying them across is the deliberate act.

A declared-change map used to sit beside the recording, letting an intended rename land green without
regenerating in the same commit. It is gone, along with the tab-separated recording it policed: byte equality
and a rename declaration are mutually exclusive — if the bytes must match, a declared rename fails anyway —
and every one of the map's rows read `=`. The
tripwire it carried survives where it is genuinely declared, as
`ValidationCharacterizationTests.BaselineRuleIdCount` (93), whose doc-comment carries the running commentary on
which rule took the count to each value.

**Six fixtures read these files** rather than re-deriving what they hold, all through one shared projection
(`tests/shared/FindingOracleHarness.cs`, linked into both consuming suites) so a change to the format is a compile
error rather than six silent breaks: `ValidationCharacterizationTests` (the byte gate and the code-count
...
`validation/*.xml` is ISO-8859-1, CRLF and BOM-less, pinned by `.gitattributes` (see above).
`tests/TestData.props` copies it into the consuming suite's output via its `**\*.xml` glob, and it is read
through the shared `FindingOracleHarness`. No `.xsd` travels with them: the schema is applied to the SOURCE
files at build time by `tests/ValidateFindingsOracles.targets`, not to the copies in an output directory. Some suites need the *source* checkout instead and go through
`TestRepository.RequireRoot()` — `SessionRefusalPolicyTests` scans the session layer's sources,
`CatalogCompletenessTests` scans `ihcclient/src` for coded origins and `CatalogTableRenderingTests` reads the
problem catalogue's markdown, among others; grep the helper for the current set rather than trusting a list
here. The findings regenerator is `[Explicit]` and writes into `findings.generated/`
beside the test binary, so the copy across is manual and deliberate.

## Notes

- Files are copied to each consuming suite's output as `testdata\...` by `tests/TestData.props`, which
  those suites `<Import>` (`*.vis` plus `*.def`/`*.ifb` for the catalog oracles, `*.html`/`*.txt` for the
  report oracles, and `*.xml` for the findings oracles). Access them through the `TestData` helper for a path
  (`projects/LiveAuthored/step02-pir2.vis`) or through `FindingOracleHarness` for a findings oracle —
  never by walking up to the source checkout. A new suite needing oracles adds the one `<Import>` line. The `.md`
  authoring records and `Project6-Error.report.pdf` are **not** copied: the glob does not take them, and
...
- All `.vis` oracles share `programmer="Morten Christensen"` and format `version 4.0`; `.vis`/`.ifb` are
  ...
  and `validation/` oracles are the exception to all of this — UTF-8 no BOM, generated rather than captured.
- Line endings are pinned in `.gitattributes`: **CRLF** for `*.vis`/`*.def`/`*.ifb` and
  `validation/*.xml`, **LF** for `reports/*`. Never renormalize or rewrite the committed blobs.
- **Captured vs. derived.** `projects/`, `products/` and `functionblocks/` are *captured* ground truth: a
  difference is the SDK's fault and the fixture is never re-saved to make a test pass. `reports/` and
  `validation/` are *derived*: a difference may be intended, but adopting it means running that oracle's own
  regeneration entry point and explaining every changed line by a change made in the same edit — never
