...
namespace Ihc.Vis.Products
{
    ...
    public static class ProductCatalogLookup
    {
        ...
        /// carry it — or carries it MORE THAN ONCE and the caller cannot say which was meant.
        /// <para><b>Identifiers are not unique.</b> Eight are shared (D22) — seven by two products, <c>_0x4408</c>
        /// by three: <c>_0x2102</c> is both <c>LK FUGA Tryk 4 tast</c> and <c>LK OPUS Tryk 4 tast</c>. Resolving
        /// with <c>FirstOrDefault</c> placed the FUGA product when the installer picked OPUS — a different
        /// product, with its own terminals, written into the project under the wrong name and noticed only when
        /// its dialog said so (T046).</para>
