...
namespace Ihc.Vis.Validation
{
    ...
    public static class DeviceSettingRules
    {
        ...
        private static void FadeZero(IProjectInspection inspection)
        {
            ...
            {
                if (Stored(product, FadeUpTag) == 0 && Stored(product, FadeDownTag) == 0)
                {
        ...
        private static void RangeInverted(IProjectInspection inspection)
        {
            foreach (ProjectElement product in Dimmers(inspection))
            {
                if (Stored(product, MinimumTag) is { } minimum && Stored(product, MaximumTag) is { } maximum
                    && minimum >= maximum)
                {
                    inspection.Report(product, Arguments(
                        ("product", Name(product)), ("minimum", minimum), ("maximum", maximum)));
                }
            }
        }
        ...
        private static void MaximumZero(IProjectInspection inspection)
        {
            foreach (ProjectElement product in Dimmers(inspection))
            {
                if (Stored(product, MaximumTag) == 0)
                {
                    inspection.Report(product, Arguments(("product", Name(product))));
                }
            }
        ...
        private static void TravelTimeZero(IProjectInspection inspection)
        {
            foreach (ProjectElement product in AllProducts(inspection.Analyses))
            {
                ...
                if (Stored(product, TravelUpTag) == 0 || Stored(product, TravelDownTag) == 0)
                {
                    inspection.Report(product, Arguments(("product", Name(product))));
                }
            }
        ...
        /// </summary>
        private static long? Stored(ProjectElement product, string tag) =>
            Setting(product, tag)?.GetAttribute("value") is { } raw
