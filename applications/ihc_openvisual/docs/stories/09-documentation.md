...
- MUST: The dialog carries two **contact** groups — **Installer** and **Customer** — each with the
  ...
  *Country*.
- MUST: All sixteen contact fields are **editable drop-downs**, each offering the values previously
  entered in that field — one list behind the installer's *Name*, another behind the customer's, and one
  shared list behind each of the other seven, so the same street/phone/postal-code/city/country/email/
  mobile values are offered on both sides. A value typed here joins that list, which is how the lists
  fill up: they are **application state**, shared across every project the installer opens and never
  written into the project file, and there is no separate editor for maintaining them.
...
- **Deliberate difference from the reference application** (registered in `product.md`, alignment F-40,
  measured 2026-08-11): the original has ONE *Dokumentation ▸ Rapporter…* entry, and it does not open a
  dialog at all — it exports the project to a temporary `.vis` and launches an **external browser** at a
  bundled `entry_page.html`, where the report is then chosen and rendered. This app gives each report type
  its own menu entry onto the shared in-app picker above, so the choice is made in the application and the
  document is generated by the engine. The original's *Rediger data tabeller…* has no counterpart either —
  editing report data tables is a registered exclusion.
...
- MUST: **Locality and description/note are disabled until a module type is chosen**, and become
  editable once one is. A line that is not in use has nothing to site or describe, so offering those
  fields would invite metadata attached to no module.
...
### Measured original behaviour (2026-08-11)

Exercised with `dialog.clickRow`, not read off a screenshot — the grid is a real `SysListView32`
(`424` inputs, 8 rows; `423` outputs, 16 rows) whose rows carry four sub-items, so it *looks* like an
editable grid and is not one:

- **No column is individually editable.** Double-clicking column 0, 1, 2 or 3 of an input row opens the
  same dialog `Indgangsmodul tilkoblet datalinie N`; an output row opens `Udgangsmodul tilkoblet
  datalinie N`. A single click only moves the selection and opens nothing. This refutes the "editable,
  clickable columns" reading that a screenshot suggested, and which was briefly registered in
  `product.md` as a deliberate difference before being withdrawn.
- **The editor is `Egenskaber`-grouped with three fields and OK/Annuller**: `Modul type` (combo,
  enabled), `Lokalitet` (combo, **disabled**), `Note` (edit, **disabled**) — the disabled pair being
  the enablement rule above, observed on a line reading `<ikke i brug>`.
- **The type list is direction-specific.** Inputs: `<ikke i brug>`, `Controller Link`, `Input 230`,
  `Input 24`, `Input 24/3`, `IR B&O Input`, `IR Indgang`. Outputs: `<ikke i brug>`, `Controller Link`,
  `Output 1-10V`, `Output 230/10`, `Output 230/16`, `Output 24`, `Output 400/10`, `Output 400/16`.
  `<ikke i brug>` is a member of both lists, which is how a line is returned to unused.
- The grid renders an unused line as `N | <ikke i brug> |  | ` — the *not in use* text sits in the
  module-type column, with locality and description blank.
...
do not yet open the module editor. Measured 2026-08-11: `dialog click-row InputModulesList --row 0
--double` reports `NoEffect`, and no column can be addressed at all because the row realizes no cells —
its whole accessible text is the single string `Datalinie 1, ikke i brug`. So the four painted column
headers sit above rows that are one flat label. Still to build: the per-row cells, the double-click
route, the editor itself and its enablement rule.
