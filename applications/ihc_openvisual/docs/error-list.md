# IHC OpenVisual — project findings catalogue (`.vis`)
...
> proposed, **79 are now settled** (68 authored, 11 refused) and the remaining 8 are classified as
...
Every condition IHC OpenVisual can report about a project file, in one catalogue, classified on three
...
| Column | Meaning |
| --- | --- |
| **Id** | Stable kebab-case rule id — the finding's identity, used for filtering, grouping and suppression. Permanent once published; a retired check keeps its id reserved. |
...
| Code | Category | Covers | Fatal | Error | Warning | Total |
| --- | --- | --- | --- | --- | --- | --- |
| **INT** | File integrity, format and transfer | Container, encoding, XML/DTD, ids, IDREFs, schema conformance, root invariants, the open/save/import/export operations | 22 | 10 | 2 | 34 |
| **WIR** | Wiring and signal flow | Follow-links between products and function blocks | — | 1 | 8 | 9 |
| **LOG** | Logic and data model | Function-block shape, programs, variables, flags, timers, enums | — | 9 | 25 | 34 |
| **SCN** | Scenes | Scene resources and their member rows | — | 1 | 7 | 8 |
...
| `load-truncated` | INT | Fatal error | Open | The document ends inside an open element | Interrupted write or transfer; the missing tail cannot be reconstructed |
...
| `name-helpfile-missing` ⊘ | DOC | Warning | — | A product or block references a help file that is not available | The help action does nothing. **Reclassified from §5:** `helpfile` is supplied by a library `.ifb` and no dialog writes or clears it — whether the file resolves is a property of the machine, not of the authoring |
...
| `struct-modified-stale` ⊘ | PRJ | Warning | — | The stored modification stamp is far older than the project's content suggests | Two versions of the project may have diverged. **Reclassified from §5:** `<modified>` is re-stamped on every save and no edit route touches it |
...
- MUST: A project with none of these conditions produces an empty list, not a "no problems found"
  pseudo-finding.
- SHOULD: Findings can be filtered or suppressed by id, by category, or by severity, so an
  installation that knowingly accepts a whole class of them can silence it without silencing the rest.
...
**⊘ Refused — reclassified to §4 (11 rows).** `dataline-address-duplicate` (OK disabled on an
...
member exists only as one half of a reciprocal pair), `name-helpfile-missing` (library-supplied; no
dialog writes or clears it), `struct-icon-default` (no element-properties dialog in the application
carries an icon picker), `struct-modified-stale` (re-stamped on every save, measured twice).
...
- the three `capacity-*` rows — out of reach at any practical fixture size.
...
The frequency column is a rough guide to which suppressions matter: an installation that does not
use identification codes or light groups silences roughly half of all findings with two ids.
