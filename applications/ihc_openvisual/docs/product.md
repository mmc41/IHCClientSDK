...
- FR-11.3: Generate one navigable project-documentation report from entered documentation, presented in the standard browser and printer-friendly, with selectable **purpose presets** (installation / technical, end-user / function, function-block, full) and **content-section and detail-option switches** the installer can turn on/off (identity, cabling & addressing cross-references, per-locality wiring & function detail, function-block logic).
- FR-11.4: Include a **documentation-completeness** report that lists, per product and terminal, what documentation is missing or inconsistent (unlinked terminal, missing identification code / light group / cable type / cable number / wire colour / placement / data-line address).
- FR-11.5: All report output is **image-free** — product identity, module addressing and wire colours are conveyed as text and tables (no product icons, module diagrams, installer logo, or external manual pictures).
