...
- **All project mutations go through the SDK's `ProjectCommands` gateway** (`ProjectAppService.Commands`) — the single authoring door: the UI obtains a ready-to-apply command from it and applies it through a session, never constructing command types or hand-editing the tree. (`ProjectEditor` / `project.Edit()` is the low-level mutation entry the gateway is built on.) All on the SDK's immutable model; the UI holds element ids, not object references.
