...
> campaign. Evidence from earlier campaigns was **not** carried over: those records did not survive
> their session, so their coverage claims can no longer be checked. A cell here is not a claim that
> nothing was ever measured — it is a claim about what can still be *shown*. That loss is the reason
> this file exists; do not recreate it by recording coverage anywhere but in a committed file.
>
...
| 12 | Dialog layout and shape | ~ | See §2. 2026-08-11 added the advanced-dimmer window (deleted 2026-08-29, T057 — its settings are fields of the product dialog now), `EnumDefinitionWindow`/`NamePromptWindow` (both sides) and the vendor SMS-modem dialog inventory (OpenVisual's own `ModemPropertiesWindow` was deleted 2026-08-12, T029 — the modem renders from a composed descriptor now) | the wired-product dialog still not field-for-field on the dataline family; `SceneContainer/SceneValue`/`ProjectInfo` not re-verified this pass |
...
| The advanced-dimmer window (*Avanceret*), deleted 2026-08-29 (T057) | ~ | ~ | ✗ | **Opened on both sides 2026-08-11** (elevated session; wireless `Lampeudtag dimmer`, fresh insert, config mode, scale 1.75). Inventories recorded. **Values match**: soft on/off 700/700 ms, min 22 %, max 100 %. **Divergences recorded:** (a) *shape* — **RESOLVED (2026-08-28):** the vendor expands the product dialog in place (group "Avancerede Dimmer egenskaber", buttons `Normal`/`OK`/`Annuller`) where OpenVisual opened a separate window titled "Avancerede lysdæmper egenskaber". Re-measured on the vendor 2026-08-28 and adopted: the six values are now ordinary fields of the composed product dialog inside a **collapsible** group carrying the vendor's own caption, committing through that dialog's own command — so the separate window, its widget slot and its command are gone. The same six values, now reached the way the original reaches them (`AdvancedDimmerGroupTests`, `ProductDialogPresetTests.Airlink_OffersAdvancedDimmerSettings_OnlyWhereDimmerSettingsExist`); (b) *ramp value* — **RESOLVED (Phase 5, 2026-08-11):** it was a real OpenVisual unit bug — `dimmer_setting_dimming_rate` is stored in **milliseconds** (default 5000, range 2000–10000) and the original shows it as **seconds**, but OpenVisual fed the raw ms into a seconds-labelled box. Fixed: the command multiplies the dialog's seconds by 1000 on write and the read divides by 1000, so 5000 ms shows as 5 s and round-trips exactly (`AdvancedDimmerGroupTests.TheManualRampRoundTripsBetweenSecondsAndMilliseconds`; the window's own command and its test went with it in T057); (c) *load-mode vocabulary* — **RESOLVED (Phase 5, 2026-08-11, owner ruling "match original vendor app"):** the combo now matches the original — **Auto detektion / RC / RL, Auto first**, label "Belastnings karakteristik" — where OpenVisual previously showed Induktiv / Kapacitiv / Auto (Auto last). Only presentation changed: the stored tokens were already the vendor `.vis` serialization `auto | rc | rl` (the dialog already wrote `rl`, which existing tests exercise), so no token or `.vis` change was needed. The `rl_led` in OpenVisual's *catalog* grammar is faithful to the vendor's own install-catalog DTD (a vendor-internal catalog-vs-project inconsistency) and was deliberately left untouched. Pinned by `AdvancedDimmerGroupTests.TheLoadTypeOffersExactlyItsDeclaredTokens`; (d) *label wording* — "Soft tænd/sluk tid (millisekunder)" vs "Blød opstart/nedlukning (ms)". 13: driven — vendor `Avanceret`→expand, `Normal`→collapse, cancel; OpenVisual `Avanceret…`→separate window, cancel, combo items enumerated live. **OK-commit not driven on either side.** 14: no invalid input submitted (T3.3) |
