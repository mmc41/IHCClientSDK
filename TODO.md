...
I want to change the implementation and testing of project reports currently generated by
applications\ihc_openvisual AND ihcclient api (ihcclient\src\app\services\ProjectAppService facade etc.)

Changes:

1) Reports can be generated in either HTML or text format (applications\ihc_openvisual\docs\icon_codes.md covers how to do textual representations of icons).

2) ihcclient api should be responsable for generating the HTML/text with the exception of icons used. ihc_openvisual is only responsbile customizing the HTML with icons. By default only textual representations (with relevant unicode icons) are used in both formats, but this can be overridden. For HTML the existing SVG icons can be swapped in instead.

3) ihcclient report api (accessed by facade) takes mimetype (text or HTML) as input and outputs a binary stream or file of the right type. the report api also takes a user supplied icon-mapping function which can be used to use SVG icons instead of (default) unicode icons etc. finally the report api should take an input describing the type of report that should be generated (functionblock, functions, installation).

4) Each of the 3 types of reports can be generated in: "Standard" and "Full" mode (both printer-friendly).
   Standard mode has the same format as the std-*.html test oracles
   Full mode has same format as the full-*.html test oracles

5) No other user options exist (which makes the existing test oracles sufficent).
