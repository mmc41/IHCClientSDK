...
| `openModal` | `{title}` of the open dialog, or null. Identified by window handle, so a dialog whose caption ends with the product name (`About IHC OpenVisual`) is still reported. |
