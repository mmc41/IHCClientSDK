...
# Open a project straight away: the first non-switch argument is a .vis path (this is also the route
# an "Open with" / double-clicked file takes — argv is the only file-activation route on Windows/Linux)
dotnet run --project applications/ihc_openvisual/ihc_openvisual.csproj -- tests/testdata/projects/Project1-SimpelWired.vis

```

A Debug run has **`.axaml` hot reload** (HotAvalonia, development-only): edit markup while the app is
running and the change is reapplied — no restart, no `dotnet watch` round trip. Nothing of it ships in
a Release build.
...
- **Layering/test rule**: view-models avoid Avalonia types so logic is testable headlessly — view-model/logic tests go in `safe_unit_tests`, headless-UI tests in `safe_visual_tests`, engine byte-fidelity **and the `Ihc.Vis.Session` command/changeset/index/session tests** in `safe_project_tests`. Shares the `ihc_lab` headless-test conventions, and the telemetry bootstrap itself via the `shared/ihc_appbootstrap` project both apps reference.
...
- **Product properties dialogs are METADATA, not windows** (2026-08-12): all five product families render through ONE `ProductDialogWindow` from a composed `ProductDialogDescriptor`. An SDK preset per family (`ProductDialogPresets`, transcribed from a recorded 100-product vendor oracle — *not* read off the DTD, which says which attributes exist rather than which the installer is offered) is resolved against the placed element by `ProductDialogComposer` into captions, current values, write targets, rules and ranges; `ApplyProductDialog` writes back pre-resolved `(target, attribute, value)` triples. **Do not add a per-family window or a per-family command — add a preset.** Four rules that are easy to break: only CHANGED fields become edits (an untouched OK is `NoChange`, and an unoffered attribute is never written); the write-back re-composes the dialog and refuses an edit to a field that family does not offer or offered read-only; the control vocabulary is small and grows only on a MEASUREMENT (`Text`/`TextMultiline`/`ComboSuggest`/`Number`, plus `Checkbox` for the one product that has one) and deliberately has no closed-list kind; `ComboSuggest` suggestions come from the OPEN PROJECT's distinct values, never from `DataTableStore`. The terminal grids, the *Avanceret* button and the sensors' *Indstillinger* grid stay hand-written markup as declared widgets, and a group of ordinary fields can be family-optional through `DialogGroupModel.PresenceTag` (the jalousi travel times) — in every case the descriptor decides whether and where they appear. `tests/testdata/projects/Synthetic/ModemDialog30Slots.vis` is a byte oracle recorded before the swap — never regenerate it to make a test pass.
- **Automation/accessibility (one tree serves screen readers AND drivers)**: every window and every interactive control needs an explicit `AutomationProperties.AutomationId` (never the silent `x:Name` fallback) plus a resolvable accessible name — `AutomationCoverageTests` walks the peer tree of the shell and all 14 dialogs and fails on any gap. Its roster is exact two-way set equality against reflection, so **adding or deleting a window is a roster edit in the same change**; the descriptor-driven dialog is additionally audited POPULATED, once per family, because an empty shell passes every accessibility check vacuously.
- **Menu ids are the `CommandRegistry` row id** (`file.new`, `edit.cut`); the node context flyout prefixes its copies `ctx.`; non-row items take a coined `menu.*` / `theme.*` / `textScale.*` id; generated catalog items publish `ProductMenuItemViewModel.AutomationId` and recent-project items `RecentProjectViewModel.AutomationId`.
- **A data-realized container publishes an id derived from its DATUM, and it must be unique among its siblings** — `AutomationCoverageTests` exempts these from the authored-markup audit, so the datum owes one: tree rows are `TreeNodeViewModel.AutomationId` (`<kind>#<element id>` — the bare kind collided across every sibling of a type), pseudo-table rows carry a row id plus a summarized accessible name (`Controls/AccessibleDataRow`, since Avalonia's Windows bridge exposes no Grid/Table pattern and a bare `Panel` is not an element at all).
- Menus use `controls:AccessibleMenu`/`AccessibleMenuItem` and trees `controls:AccessibleTreeView` — Avalonia's stock peers expose no Invoke/ExpandCollapse, so plain `MenuItem`/`TreeViewItem` cannot be operated through UI Automation at all. Never author a bare `<MenuItem>` in this app.
