...
- **Automation/accessibility (one tree serves screen readers AND drivers)**: every window and every interactive control needs an explicit `AutomationProperties.AutomationId` (never the silent `x:Name` fallback) plus a resolvable accessible name — `AutomationCoverageTests` walks the peer tree of the shell and all 16 dialogs and fails on any gap.
